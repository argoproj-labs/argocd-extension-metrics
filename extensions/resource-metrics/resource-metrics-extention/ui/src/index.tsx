    ...
    <React.Fragment>
      <div className='application-metrics__MetricsDurationSelector'>
        <a
          ...
          onClick={(e)=>{updateDuration(e, "24h")}}>
            1 day
        </a>
        <a
          href={`${loc}`}
          className={`application-metrics__MetricsDuration ${duration === '12h' ? 'active' : ''}`}
          onClick={(e)=>{updateDuration(e, "12h")}}
        >
            12 hrs
        </a>
        <a
          href={`${loc}`}
          className={`application-metrics__MetricsDuration ${duration === '6h' ? 'active' : ''}`}
          onClick={(e)=>{updateDuration(e, "6h")}}
        >
            6 hrs
        </a>
        <a
          href={`${loc}`}
          className={`application-metrics__MetricsDuration ${duration === '2h' ? 'active' : ''}`}
          onClick={(e)=>{updateDuration(e, "2h")}}
        >
            2 hrs
        </a>
        <a
          href={`${loc}`}
          className={`application-metrics__MetricsDuration ${duration === '1h' ? 'active' : ''}`}
          onClick={(e)=>{updateDuration(e, "1h")}}
        >
            1 hr
        </a>
      </div>
...
((window: any) => {
  window?.extensionsAPI?.registerResourceExtension(component, '*', '*', 'Metrics', {icon: "fa fa-chart-area"});
